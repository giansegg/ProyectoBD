...
def create_all(n, schema) -> None:
    ...
    def genCasos():
        ...
        while i < n:
            try:
                ...
                tipo_caso = fake.random_element(elements=("Civil", "Penal", "Laboral", "Familiar", "Mercantil"))
                cursor.execute(
                    f"SELECT 1 FROM {schema}.Caso WHERE (codigo) = ('{codigo}');")
                if not cursor.fetchone():
                    cursor.execute(
    ...
    def genTelefonos():
        i = 0
        cursor.execute(f"SELECT tipo_documento, numero_documento FROM {schema}.Persona;")
        res = cursor.fetchall()
        while i < 0.5*n:
            try:
                ...
                cursor.execute(
                    f"SELECT 1 FROM {schema}.Telefono WHERE (numero) = ('{numero}');")
                if not cursor.fetchone():
                    cursor.execute(
    ...
    def genEmpleados():
        ...
        while i < n*0.8:
            try:
                ...
                cursor.execute(
                    f"SELECT 1 FROM {schema}.Empleado WHERE (tipo_documento, numero_documento) = ('{fk[0]}','{fk[1]}');")
                if not cursor.fetchone():
                    cursor.execute(
    ...
    def genAbogados():
        i = 0
        cursor.execute(f"SELECT tipo_documento, numero_documento FROM {schema}.Empleado;")
        res = cursor.fetchall()
        while i < n*0.7*0.8:
            try:
                fk = random.choice(res)
                ...
                cursor.execute(
                    f"SELECT 1 FROM {schema}.Abogado WHERE (tipo_documento, numero_documento) = ('{fk[0]}','{fk[1]}');")
                if not cursor.fetchone():
                    cursor.execute(
                        ...
                        (fk[1], fk[0], especializacion, numero_colegiatura, casos_ganados, casos_perdidos)
                    )
                    i += 1
            except Exception as e:
                pass
    ...
    def genDepartamentos():
        i = 0
        cursor.execute(f"SELECT tipo_documento, numero_documento FROM {schema}.Abogado;")
        res = cursor.fetchall()
    ...
    def genAbogadoTrabaja():
        ...
        while i < 0.8*n:
            try:
                ...
                fecha_creacion = fake.date_time_between(start_date="-5y", end_date="now")
                cursor.execute(
                    f"SELECT 1 FROM {schema}.AbogadoTrabaja WHERE (tipo_documento_abogado, numero_documento_abogado, nombre_departamento) = ('{fk[0]}','{fk[1]}', '{nombre}');")
                if not cursor.fetchone():
                    cursor.execute(
    ...
    def genSecretarioAsiste():
        ...
        cursor.execute(f"SELECT tipo_documento, numero_documento FROM {schema}.Abogado;")
        res = cursor.fetchall()
        cursor.execute(f"SELECT tipo_documento, numero_documento FROM {schema}.Secretario;")
        ...
        while i < 0.6*0.8*n:
            try:
                ...
                cursor.execute(
                    f"SELECT 1 FROM {schema}.SecretarioAsiste WHERE (tipo_documento_abogado, numero_documento_abogado) = ('{fk[0]}','{fk[1]}');")
                if not cursor.fetchone():
                    cursor.execute(
    ...
    def genPersonaParticipa():
        ...
        while i < 0.5*n:
            try:
                ...
                cursor.execute(
                    f"SELECT 1 FROM {schema}.PersonaParticipa WHERE (tipo_documento, numero_documento, caso_codigo) = ('{fk[0]}','{fk[1]}','{fk2[0]}');")
                if not cursor.fetchone():
                    cursor.execute(
    ...
    def genPersonaRepresenta():
        ...
        while i < 0.5*n:
            try:
                ...
                cursor.execute(
                    f"SELECT 1 FROM {schema}.PersonaRepresenta WHERE (tipo_documento, numero_documento, ruc, caso_codigo) = ('{fk[0]}','{fk[1]}','{fk2[0]}','{fk3[0]}');")
                if not cursor.fetchone():
                    cursor.execute(
    ...
    def genAbogadoParticipa():
        ...
        while i < 0.8*n:
            try:
                ...
                fk2 = random.choice(res2)
                cursor.execute(
                    f"SELECT 1 FROM {schema}.AbogadoParticipa WHERE (tipo_documento_abogado, numero_documento_abogado, caso_codigo) = ('{fk[0]}','{fk[1]}','{fk2[0]}');")
                if not cursor.fetchone():
                    cursor.execute(
    ...
    def genDocumento():
        ...
        while i < 0.5*n:
            try:
                ...
                nombre = fake.file_name(extension=random.choice(["pdf", "csv", "xlsx", "docs", "cpp", "py"]))
